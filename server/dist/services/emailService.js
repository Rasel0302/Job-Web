...
class EmailService {
    ...
    async sendEmail(options) {
        try {
            ...
            await this.logEmail(options, false, error.message);
            return false;
        }
    }
    async logEmail(options, isSent, errorMessage) {
        try {
            const connection = getConnection();
            await connection.execute('INSERT INTO email_notifications (recipient_email, subject, body, type, is_sent, error_message) VALUES (?, ?, ?, ?, ?, ?)', [
                options.to,
                options.subject,
                options.html,
    ...
    async sendInvitationEmail(email, inviterName, companyName, inviteLink) {
    ...
    async sendApplicationStatusUpdate(email, jobTitle, companyName, status) {
    ...
    async sendJobMatchNotification(email, jobTitle, companyName, matchScore) {
    ...
    generateWelcomeTemplate(firstName, role) {
        return `
      <!DOCTYPE html>
      <html>
      <head>
    ...
    generateInvitationTemplate(inviterName, companyName, inviteLink) {
        return `
      <!DOCTYPE html>
      <html>
      <head>
    ...
    generateApplicationStatusTemplate(jobTitle, companyName, status) {
        const statusColors = {
            'reviewed': '#f59e0b',
            'interviewed': '#3b82f6',
            'accepted': '#10b981',
    ...
    generateJobMatchTemplate(jobTitle, companyName, matchScore) {
        return `
      <!DOCTYPE html>
      <html>
      <head>
    ...
    async sendApprovalEmail(email, userType, approved, reason) {
