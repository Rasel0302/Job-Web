...
export class JobRecommendationService {
    ...
    static async getUserProfile(userId) {
      ...
      `, [userId]);
                ...
                if (resume.skills) {
                    try {
                        ...
                        if (Array.isArray(skillsData)) {
                            skills.push(...skillsData.map((skill) => skill.name || skill).filter(Boolean));
                        }
                    }
                    catch (e) {
            ...
            const profile = {
                id: user.id,
                skills: skills,
                courses: userCourses.map(c => c.course_name),
    ...
    static calculateJobMatch(userProfile, job) {
        let score = 0;
        const reasons = [];
        const maxScore = 100;
    ...
    static getCourseJobMatch(userCourses, jobCategory) {
        ...
        for (const course of userCourses) {
            ...
            if (matchingCategories.some(cat => cat.toLowerCase().includes(jobCategory.toLowerCase()) ||
    ...
    static getSkillsMatch(userSkills, jobTitle, jobDescription) {
    ...
    static getExperienceMatch(userProfile, jobExperienceLevel) {
    ...
    static getStudentTypeMatch(studentType, workType) {
        if (studentType === 'ojt') {
