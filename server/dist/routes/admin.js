...
router.get('/pending-approvals', authenticate, authorize('admin'), asyncHandler(async (req, res) => {
...
// Approve user
router.post('/approve/:type/:id', authenticate, authorize('admin'), asyncHandler(async (req, res) => {
    ...
    const [user] = await connection.execute(`SELECT id, email FROM ${tableName} WHERE id = ? AND is_verified = TRUE AND is_approved = FALSE`, [id]);
    if (user.length === 0) {
    ...
    if (type === 'admin') {
        const [adminProfile] = await connection.execute('SELECT is_profile_complete FROM admin_profiles WHERE admin_id = ?', [id]);
        if (adminProfile.length === 0 || !adminProfile[0].is_profile_complete) {
        ...
        const [coordinatorProfile] = await connection.execute('SELECT is_profile_complete FROM coordinator_profiles WHERE coordinator_id = ?', [id]);
        if (coordinatorProfile.length === 0 || !coordinatorProfile[0].is_profile_complete) {
            return res.status(400).json({
    ...
    // Approve user
    await connection.execute(`UPDATE ${tableName} SET is_approved = TRUE WHERE id = ?`, [id]);
...
// Reject user
router.post('/reject/:type/:id', authenticate, authorize('admin'), asyncHandler(async (req, res) => {
    const { type, id } = req.params;
    ...
    const [user] = await connection.execute(`SELECT id, email FROM ${tableName} WHERE id = ?`, [id]);
    if (user.length === 0) {
...
router.get('/coordinators', authenticate, authorize('admin'), asyncHandler(async (req, res) => {
    const connection = getConnection();
    const [coordinators] = await connection.execute(`
    SELECT 
      c.id,
...
// Get all companies
router.get('/companies', authenticate, authorize('admin'), asyncHandler(async (req, res) => {
    const connection = getConnection();
    const [companies] = await connection.execute(`
    SELECT 
      c.id,
...
router.get('/admins', authenticate, authorize('admin'), asyncHandler(async (req, res) => {
    const connection = getConnection();
    const [admins] = await connection.execute(`
    SELECT 
      a.id,
...
// Get all jobs
router.get('/jobs', authenticate, authorize('admin'), asyncHandler(async (req, res) => {
    const connection = getConnection();
...
router.post('/complete-profile', authenticateForProfileCompletion, asyncHandler(async (req, res) => {
    const { firstName, lastName, contactNumber, age, birthdate, gender, position, department, profilePhotoUrl } = req.body;
    if (!firstName || !lastName || !contactNumber || !age || !birthdate || !gender || !position || !profilePhotoUrl) {
        return res.status(400).json({ message: 'All required fields must be provided' });
    }
    const connection = getConnection();
    // Check if profile already exists
    const [existingProfile] = await connection.execute('SELECT id FROM admin_profiles WHERE admin_id = ?', [req.user.id]);
    if (existingProfile.length > 0) {
        // Update existing profile
        await connection.execute(`UPDATE admin_profiles SET 
       ...
       WHERE admin_id = ?`, [firstName, lastName, contactNumber, age, birthdate, gender, position, department, profilePhotoUrl, req.user.id]);
    }
    else {
        // Create new profile
        await connection.execute(`INSERT INTO admin_profiles 
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, TRUE)`, [req.user.id, firstName, lastName, contactNumber, age, birthdate, gender, position, department, profilePhotoUrl]);
    }
    res.json({
...
router.put('/profile', authenticate, authorize('admin'), asyncHandler(async (req, res) => {
    const { firstName, lastName, contactNumber, age, birthdate, gender, position, department, profilePhotoUrl } = req.body;
    const connection = getConnection();
    ...
    if (existingProfile.length > 0) {
        ...
        if (profilePhotoUrl) {
         ...
         WHERE admin_id = ?`, [firstName, lastName, contactNumber, age, birthdate, gender, position, department, req.user.id]);
        }
    }
    else {
        // Create new profile
...
router.delete('/admins/:id', authenticate, authorize('admin'), asyncHandler(async (req, res) => {
    const { id } = req.params;
    ...
    const [admin] = await connection.execute('SELECT id, email FROM admins WHERE id = ?', [id]);
    if (admin.length === 0) {
        return res.status(404).json({ message: 'Admin not found' });
    }
    // Check if there's at least one other admin remaining
    const [adminCount] = await connection.execute('SELECT COUNT(*) as count FROM admins WHERE is_approved = TRUE AND id != ?', [id]);
    ...
    await connection.execute('DELETE FROM admin_profiles WHERE admin_id = ?', [id]);
