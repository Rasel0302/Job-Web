...
import { hashPassword, comparePassword, generateJWT, generateOTP, generateToken, validatePassword, validateAsiatechEmail, validateEmail } from '../utils/auth.js';
...
router.post('/register/user', asyncHandler(async (req, res) => {
    ...
    // Create user
    const [result] = await connection.execute('INSERT INTO users (email, password_hash, verification_token) VALUES (?, ?, ?)', [email, hashedPassword, verificationToken]);
    // Send OTP email
...
router.post('/register/coordinator', asyncHandler(async (req, res) => {
    ...
    const [existingCoordinators] = await connection.execute('SELECT id FROM coordinators WHERE email = ?', [email]);
    ...
    // Create coordinator
    const [result] = await connection.execute('INSERT INTO coordinators (email, password_hash, verification_token) VALUES (?, ?, ?)', [email, hashedPassword, verificationToken]);
    // Send OTP email
...
router.post('/register/admin', asyncHandler(async (req, res) => {
    ...
    const [existingAdmins] = await connection.execute('SELECT id FROM admins WHERE email = ?', [email]);
    ...
    const [result] = await connection.execute('INSERT INTO admins (email, password_hash, verification_token, is_verified, is_approved) VALUES (?, ?, ?, ?, ?)', [email, hashedPassword, verificationToken, false, false]);
    const adminId = result.insertId;
    // Create admin profile
    await connection.execute('INSERT INTO admin_profiles (admin_id, first_name, last_name) VALUES (?, ?, ?)', [adminId, firstName, lastName]);
    // Send OTP email
...
router.post('/verify-otp', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const [allOtps] = await connection.execute('SELECT * FROM otp_verifications WHERE email = ? ORDER BY created_at DESC', [email]);
        ...
        const [records] = await connection.execute('SELECT * FROM otp_verifications WHERE email = ? AND otp_code = ? AND purpose = ? AND is_used = FALSE ORDER BY created_at DESC LIMIT 1', [email, otp, purpose]);
        otpRecords = records;
        ...
        // Mark OTP as used
        await connection.execute('UPDATE otp_verifications SET is_used = TRUE WHERE id = ?', [otpRecord.id]);
...
router.post('/login', asyncHandler(async (req, res) => {
    ...
    if (validateAsiatechEmail(email)) {
        const [users] = await connection.execute('SELECT id, email, password_hash, is_verified FROM users WHERE email = ?', [email]);
        if (users.length > 0) {
            user = users[0];
            tableName = 'users';
            userRole = 'user';
        ...
        for (const table of tables) {
            const [results] = await connection.execute(`SELECT id, email, password_hash, is_verified, is_approved FROM ${table.name} WHERE email = ?`, [email]);
            if (results.length > 0) {
                user = results[0];
...
router.post('/resend-otp', asyncHandler(async (req, res) => {
    ...
    const otpExpires = new Date(Date.now() + 10 * 60 * 1000);
    // Store new OTP
    await connection.execute('INSERT INTO otp_verifications (email, otp_code, purpose, expires_at) VALUES (?, ?, ?, ?)', [email, otp, purpose, otpExpires]);
    // Send OTP email
...
router.post('/forgot-password', asyncHandler(async (req, res) => {
    ...
    for (const table of tables) {
        const [results] = await connection.execute(`SELECT id FROM ${table} WHERE email = ?`, [email]);
        if (results.length > 0) {
            userExists = true;
            tableName = table;
            break;
    ...
    const otpExpires = new Date(Date.now() + 10 * 60 * 1000); // 10 minutes
    // Store reset token
    await connection.execute(`UPDATE ${tableName} SET reset_token = ?, reset_token_expires = ? WHERE email = ?`, [resetToken, resetExpires, email]);
    // Store OTP
    await connection.execute('INSERT INTO otp_verifications (email, otp_code, purpose, expires_at) VALUES (?, ?, ?, ?)', [email, otp, 'password_reset', otpExpires]);
    // Send OTP email
...
router.post('/reset-password', asyncHandler(async (req, res) => {
    ...
    const connection = getConnection();
    // Verify OTP
    const [otpRecords] = await connection.execute('SELECT * FROM otp_verifications WHERE email = ? AND otp_code = ? AND purpose = ? AND expires_at > NOW() AND is_used = FALSE', [email, otp, 'password_reset']);
    if (otpRecords.length === 0) {
        return res.status(400).json({ message: 'Invalid or expired OTP' });
    }
    ...
    const tables = ['users', 'coordinators', 'companies', 'admins'];
    for (const table of tables) {
        const [results] = await connection.execute(`SELECT id FROM ${table} WHERE email = ? AND reset_token = ? AND reset_token_expires > NOW()`, [email, resetToken]);
        if (results.length > 0) {
            userTable = table;
            break;
        }
    ...
    await connection.execute(`UPDATE ${userTable} SET password_hash = ?, reset_token = NULL, reset_token_expires = NULL WHERE email = ?`, [hashedPassword, email]);
    // Mark OTP as used
    await connection.execute('UPDATE otp_verifications SET is_used = TRUE WHERE id = ?', [otpRecords[0].id]);
    res.json({
