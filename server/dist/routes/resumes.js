...
router.get('/templates', authenticate, authorize('user'), asyncHandler(async (req, res) => {
...
router.get('/', authenticate, authorize('user'), asyncHandler(async (req, res) => {
  ...
  `;
    ...
    const resumesWithTemplates = resumes.map(resume => ({
        ...resume,
        template: RESUME_TEMPLATES[resume.template_id] || null
    }));
...
router.get('/:id', authenticate, authorize('user'), asyncHandler(async (req, res) => {
  ...
  `, [id, req.user.id]);
    ...
    const parsedResume = {
        ...
        template: RESUME_TEMPLATES[resumeData.template_id] || null
    };
    res.json(parsedResume);
}));
// Create resume
router.post('/', authenticate, authorize('user'), asyncHandler(async (req, res) => {
    const { title, templateId, personalInfo, professionalSummary, workExperience, education, skills, websitesSocialLinks, customSections, extracurricularActivities, hobbies, references, languages, fontFamily, paperSize, isPrimary = false } = req.body;
    if (!title || !templateId) {
        return res.status(400).json({ message: 'Resume title and template are required' });
    }
    if (!RESUME_TEMPLATES[templateId]) {
        return res.status(400).json({ message: 'Invalid template ID' });
    }
    const connection = getConnection();
    // If setting as primary, unset other primary resumes
    if (isPrimary) {
        await connection.execute('UPDATE resumes SET is_primary = FALSE WHERE user_id = ?', [req.user.id]);
    }
  ...
  `, [
    ...
    res.status(201).json({
        ...
        resume: {
            ...
            status: 'draft',
            template: RESUME_TEMPLATES[templateId]
        }
    });
}));
// Update resume (full update)
router.put('/:id', authenticate, authorize('user'), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { title, templateId, personalInfo, professionalSummary, workExperience, education, skills, websitesSocialLinks, customSections, extracurricularActivities, hobbies, references, languages, fontFamily, paperSize, status, isPrimary } = req.body;
    const connection = getConnection();
    // Verify resume belongs to user
    const [resume] = await connection.execute('SELECT id, template_id FROM resumes WHERE id = ? AND user_id = ?', [id, req.user.id]);
    if (resume.length === 0) {
        return res.status(404).json({ message: 'Resume not found' });
    }
    ...
    // If setting as primary, unset other primary resumes
    if (isPrimary) {
        await connection.execute('UPDATE resumes SET is_primary = FALSE WHERE user_id = ? AND id != ?', [req.user.id, id]);
    }
...
router.patch('/:id/autosave', authenticate, authorize('user'), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const updates = req.body;
    const connection = getConnection();
    // Verify resume belongs to user
    const [resume] = await connection.execute('SELECT id FROM resumes WHERE id = ? AND user_id = ?', [id, req.user.id]);
    if (resume.length === 0) {
        return res.status(404).json({ message: 'Resume not found' });
    }
...
// Delete resume
router.delete('/:id', authenticate, authorize('user'), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const connection = getConnection();
    // Verify resume belongs to user
    const [resume] = await connection.execute('SELECT id FROM resumes WHERE id = ? AND user_id = ?', [id, req.user.id]);
    ...
    await connection.execute('DELETE FROM resumes WHERE id = ? AND user_id = ?', [id, req.user.id]);
    res.json({ message: 'Resume deleted successfully' });
}));
// Set primary resume
router.patch('/:id/primary', authenticate, authorize('user'), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const connection = getConnection();
    // Verify resume belongs to user
    const [resume] = await connection.execute('SELECT id FROM resumes WHERE id = ? AND user_id = ?', [id, req.user.id]);
    ...
    await connection.execute('UPDATE resumes SET is_primary = TRUE WHERE id = ? AND user_id = ?', [id, req.user.id]);
    res.json({ message: 'Primary resume updated successfully' });
}));
// Mark resume as completed
router.patch('/:id/complete', authenticate, authorize('user'), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const connection = getConnection();
    // Verify resume belongs to user
    const [resume] = await connection.execute('SELECT id FROM resumes WHERE id = ? AND user_id = ?', [id, req.user.id]);
    ...
    await connection.execute('UPDATE resumes SET status = "completed", updated_at = NOW() WHERE id = ? AND user_id = ?', [id, req.user.id]);
    res.json({ message: 'Resume marked as completed' });
}));
// Download resume as PDF
router.get('/:id/download', authenticate, authorize('user'), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const connection = getConnection();
    // Verify resume belongs to user and get data
    const [resume] = await connection.execute('SELECT * FROM resumes WHERE id = ? AND user_id = ?', [id, req.user.id]);
    if (resume.length === 0) {
        return res.status(404).json({ message: 'Resume not found' });
    }
    const resumeData = resume[0];
    // Update download count
    await connection.execute('UPDATE resumes SET download_count = download_count + 1, last_downloaded = NOW() WHERE id = ? AND user_id = ?', [id, req.user.id]);
...
// Duplicate resume
router.post('/:id/duplicate', authenticate, authorize('user'), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { title } = req.body;
    const connection = getConnection();
    // Get original resume
    const [originalResume] = await connection.execute('SELECT * FROM resumes WHERE id = ? AND user_id = ?', [id, req.user.id]);
    if (originalResume.length === 0) {
        return res.status(404).json({ message: 'Resume not found' });
    }
    const original = originalResume[0];
  ...
  `, [
    ...
    res.status(201).json({
        ...
        resume: {
            ...
            status: 'draft',
            template: RESUME_TEMPLATES[original.template_id]
        }
    });
}));
